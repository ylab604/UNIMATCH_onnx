...
def single_head_full_attention_1d(q, k, v,
    ...
    scores = torch.matmul(q, k.permute(0, 1, 3, 2)) / (128**0.5)
...
def single_head_split_window_attention(q, k, v,
    ...
    if with_shift:
        print("single_head_split_window_attention shift check 1")
    ...
    scores = torch.matmul(q.view(b_new, -1, c), k.view(b_new, -1, c).permute(0, 2, 1)
                          ) / 128**0.5  # [B*K*K, H/K*W/K, H/K*W/K]

    # if with_shift:
    #     print("single_head_split_window_attention shift check 2")
    ...
    if with_shift:
        ...
        # print(scores.shape) #torch.Size([8, 1200, 1200])
        # print(attn_mask.shape) #torch.Size([1, 4, 1200, 1200])
...
def single_head_split_window_attention_1d(q, k, v,
    ...
    if with_shift:
        print("single_head_split_window_attention_1d 11")
        assert attn_mask is not None  # compute once
        shift_size_w = window_size_w // 2
        shift_size_w = int(shift_size_w)  # Tensor를 정수로 변환
    ...
    # shift back
    if with_shift:
        print("single_head_split_window_attention_1d 22")
...
class SelfAttnPropagation(nn.Module):
    ...
    def forward(self, feature0, flow,
        ...
        if local_window_attn:
            print("check local_window_attn chechecehk")
        ...
        scores = torch.matmul(query, key.permute(0, 2, 1)) / (128 ** 0.5)  # [B, H*W, H*W]
