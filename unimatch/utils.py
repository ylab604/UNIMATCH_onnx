...
#     img_mask = torch.zeros((1, h, w, 1)).to(device)  # 1 H W 1
#     cnt = 0
#     for i, (h_start, h_end) in enumerate([(0, -window_size_h), (-window_size_h, -shift_size_h), (-shift_size_h, None)]):
#         for j, (w_start, w_end) in enumerate(
#                 [(0, -window_size_w), (-window_size_w, -shift_size_w), (-shift_size_w, None)]):
#             if h_end is None:
#                 h_end = img_mask.size(1)
#             if w_end is None:
#                 w_end = img_mask.size(2)
#             img_mask[:, h_start:h_end, w_start:w_end, :] = cnt
#             cnt += 1
#
#
#
...
def feature_add_position(feature0, feature1, attn_splits, feature_channels):
    ...
    # print(feature0.shape) #torch.Size([1, 128, 60, 80])
    # print(feature1.shape) #torch.Size([1, 128, 60, 80])

    # if attn_splits > 1:  # add position in splited window
    feature0_splits = split_feature(feature0, num_splits=attn_splits)
    feature1_splits = split_feature(feature1, num_splits=attn_splits)
    # print(feature0_splits.shape) #torch.Size([4, 128, 30, 40])
    # print(feature1_splits.shape) #torch.Size([4, 128, 30, 40])

    position = pos_enc(feature0_splits)
    # print(position.shape) torch.Size([4, 128, 30, 40])
    # print("check the position shape@@@@@@@@@")
    ...
    # print("check the feature0, feature1 shape")
    # else:
    ...
    # print(feature1.shape) #torch.Size([1, 128, 60, 80])
    # print(feature0.shape) #torch.Size([1, 128, 60, 80])
    # print("check the final feature  shape!!!!!!!!!!!!!!!")
...
def upsample_flow_with_mask(flow, up_mask, upsample_factor,
    ...
    # print("@!#!@#!@#!@#check the up_flow")
    # print(up_flow.shape) torch.Size([1, 18, 4800])
    # print("@!#!@#!@#!@#check the up_flow")
...
#     # print("generate_shift_window_attn_mask_1d@@@@@@@@@@@@@@@@@@@@")
#     # calculate attention mask for SW-MSA
...
#     #     cnt += 1
#
#     img_mask = torch.zeros((1, input_w, 1), device=device)  # 초기 마스크 생성
#
#     # 정적 인덱스를 생성하여 각 윈도우에 대한 마스크 값 할당
#     indices = torch.arange(input_w, device=device).view(1, input_w, 1)  # 1D 인덱스 텐서 생성
#     img_mask = torch.zeros_like(indices, device=device)  # 같은 크기의 마스크 텐서 초기화
#     img_mask.fill_(0)
#
